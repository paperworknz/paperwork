...
Form.prototype.update = function(){
	...
	$(a.s).find('.qq-total').each(function(){
		var item = $(this).parent().attr('data-item'),
			qty = $(a.s).find('[data-item="'+item+'"] .qq-qty').val(),
			price = $(a.s).find('[data-item="'+item+'"] .qq-price').val().replace('$', '');
		
		// Standardize data
		var nprice = std(Number(price)); // Standardize price to a number with two decimal places
		
		// Total
		var total = std((Number(qty) * nprice));
		i += Number(total);
		...
		// Update DOM
		$(a.s).find('[data-item="'+item+'"] .qq-price').val('$'+price); // Input price
		$(a.s).find('[data-item="'+item+'"] .qq-price').next().html(price); // Hidden price
	...
	var subtotal = std(i),
		tax = std((((i * 3) / 2) / 10));
	$(a.s).find('[form-subtotal]').html('$'+subtotal);	// Update sub-total
	$(a.s).find('[form-tax]').html('$'+tax);			// Tax 15%
	$(a.s).find('[form-total]').html('$'+std((Number(subtotal)+Number(tax)))); // Total
};
